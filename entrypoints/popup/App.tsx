...
export function App() {
  ...
  async function loadDetection() {
    try {
      ...
      if (response?.detected) {
        setState({ status: "detected", detection: response as DetectionResult });
      }
    } catch {
  ...
  async function handleAnalyze() {
    ...
    if (!policyText) {
      setState({ status: "error", message: "No policy text found on this page." });
      return;
    }
    ...
    <div class={styles.container}>
      ...
      <main class={styles.main}>
        ...
        {state.status === "detected" && (
          <DetectedView detection={state.detection} onAnalyze={handleAnalyze} />
        )}
  ...
  return (
    <div class={styles.stateView}>
      ...
      <p class={styles.idleHint}>
        Visit a page with a privacy policy and this extension will alert you.
      </p>
    </div>
  );
}
...
function DetectedView({
  ...
  return (
    <div class={styles.stateView}>
      <p class={styles.detectedText}>Policy content detected on this page.</p>
      <p class={styles.meta}>
...
function ResultsView({
  ...
  return (
    <div class={styles.results}>
      <div class={styles.gradeSection} style={{ borderColor: gradeColor }}>
          ...
          <ul class={styles.redFlagList}>
            {analysis.redFlags.map((flag, i) => (
              <li key={i} class={styles.redFlag}>
                ...
                />
                <div>
                  <p class={styles.flagSummary}>{flag.summary}</p>
      ...
      <details class={styles.expandable}>
        ...
        <dl class={styles.detailList}>
          <dt>Collected</dt>
          <dd>{analysis.dataPractices.collected.join(", ") || "Not specified"}</dd>
          <dt>Usage</dt>
          <dd>{analysis.dataPractices.usage.join(", ") || "Not specified"}</dd>
          <dt>Shared With</dt>
          <dd>{analysis.dataPractices.sharedWith.join(", ") || "Not specified"}</dd>
          <dt>Retention</dt>
